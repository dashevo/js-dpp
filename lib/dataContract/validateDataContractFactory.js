...
const IndexPropertyIsObjectError = require('../errors/IndexPropertyIsObjectError');
      ...
      .forEach(([documentType, documentSchema]) => {
        ...
        documentSchema.indices.forEach((indexDefinition) => {
          ...
          indexPropertyNames.forEach((propertyName) => {
            const { type: propertyType } = (documentSchema.properties[propertyName] || {});

            if (propertyType === 'object') {
              result.addError(new IndexPropertyIsObjectError(
                rawDataContract,
                documentType,
                indexDefinition,
                propertyName,
