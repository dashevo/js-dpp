...
function validateDataContractCreateTransitionStructureFactory(
  ...
  async function validateDataContractCreateTransitionStructure(stateTransitionJson) {
    ...
    result.merge(
      await validateDataContract(stateTransition.getDataContract()),
    );
    ...
    if (!entropy.validate(bs58.decode(stateTransitionJson.entropy))) {
      result.addError(
    ...
    const generatedId = generateDataContractId(
      dataContract.getOwnerId().toBuffer(), bs58.decode(stateTransitionJson.entropy),
    );

    if (Buffer.compare(generatedId, dataContractId.toBuffer()) !== 0) {
      result.addError(
    ...
    // Verify ST signature
    result.merge(
      await validateStateTransitionSignature(stateTransition, dataContract.getOwnerId().toBuffer()),
    );
