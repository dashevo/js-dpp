...
const InvalidDPObjectStateError = require('./errors/InvalidDPObjectStateError');
...
class DPObject {
  ...
  setAction(action) {
    if (action === DPObject.ACTIONS.DELETE && Object.keys(this.data).length !== 0) {
      throw new InvalidDPObjectStateError(
        InvalidDPObjectStateError.CODES.DATA_IS_SET,
        this,
      );
    }
  ...
  set(path, value) {
    if (this.action === DPObject.ACTIONS.DELETE) {
      throw new InvalidDPObjectStateError(
        InvalidDPObjectStateError.CODES.ACTION_IS_DELETE,
        this,
      );
    }
