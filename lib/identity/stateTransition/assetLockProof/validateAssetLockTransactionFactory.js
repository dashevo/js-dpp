...
const AssetLockTransactionOutputNoReturnError = require('../../../errors/consensus/basic/identity/InvalidIdentityAssetLockTransactionOutputError');
...
function validateAssetLockTransactionFactory(stateRepository) {
  ...
  async function validateAssetLockTransaction(rawTransaction, outputIndex) {
    ...
    if (!output.script.isDataOut()) {
      result.addError(
        new AssetLockTransactionOutputNoReturnError(outputIndex),
      );
