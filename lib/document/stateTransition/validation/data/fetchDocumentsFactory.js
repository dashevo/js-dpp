...
function fetchDocumentsFactory(dataProvider) {
   ...
   *        |DocumentDeleteTransition[]} documentActionTransitions
   * @return {Document[]}
   */
  async function fetchDocuments(dataContractId, documentActionTransitions) {
    // Group Document IDs by types
    const documentIdsByTypes = documentActionTransitions.reduce((obj, document) => {
