...
const InvalidIdentityPublicKeyType = require('../../../../errors/InvalidIdentityPublicKeyType');
...
function validateDocumentsSTDataFactory(
  ...
  async function validateDocumentsSTData(stateTransition) {
    ...
    if (publicKey.getType() !== IdentityPublicKey.TYPES.ECDSA_SECP256K1) {
      result.addError(
        new InvalidIdentityPublicKeyType(publicKey.getType()),
      );

      return result;
    }

    if (!stateTransition.getSignature()) {
      result.addError();
    }
