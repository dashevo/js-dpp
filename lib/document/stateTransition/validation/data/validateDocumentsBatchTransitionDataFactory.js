...
const DocumentTimestampsAreNotSetError = require(
  '../../../../errors/DocumentTimestampsAreNotSetError',
);
const DocumentTimestampWindowViolationError = require(
  '../../../../errors/DocumentTimestampWindowViolationError',
);
const DocumentTimestampWasAlteredError = require(
  '../../../../errors/DocumentTimestampWasAlteredError',
);
      ...
      .forEach((documentTransition) => {
        ...
        switch (documentTransition.getAction()) {
          case AbstractDocumentTransition.ACTIONS.CREATE:
            ...
            if (documentTransition.getCreatedAt() === undefined || documentTransition.getUpdatedAt() === undefined) {
              result.addError(new DocumentTimestampsAreNotSetError(documentTransition));
            } else {
              // Check they are equal
              if (documentTransition.getCreatedAt().getTime() !== documentTransition.getUpdatedAt().getTime()) {
                result.addError(new DocumentTimestampsMismatchError(documentTransition));
              }

              // Check timestamp is within a block time window
              const createdAtTime = documentTransition.getCreatedAt().getTime();
              if (createdAtTime < timeWindowStart || createdAtTime > timeWindowEnd) {
                result.addError(
                  new DocumentTimestampWindowViolationError(documentTransition, fetchedDocument),
                );
              }
            }
          ...
          case AbstractDocumentTransition.ACTIONS.REPLACE: {
            ...
            if (documentTransition.getCreatedAt() === undefined || documentTransition.getUpdatedAt() === undefined) {
              ...
              // Check createAd have not been altered
              if (fetchedDocument && createdAtTime !== fetchedDocument.getCreatedAt()) {
                result.addError(
                  new DocumentTimestampWasAlteredError(documentTransition, fetchedDocument),
                );
              }
