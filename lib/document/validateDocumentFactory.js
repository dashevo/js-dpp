...
module.exports = function validateDocumentFactory(
   ...
   * @param {Object} [options={allowMeta: true}]
  ...
  function validateDocument(document, contract, options = { allowMeta: true }) {
    ...
    if (rawDocument.$action === Document.ACTIONS.DELETE) {
      ...
      if (!options.allowMeta) {
        for (let i = 0; i < Object.keys(enrichedContract.documents).length; i++) {
          const enrichedDocumentName = Object.keys(enrichedContract.documents)[i];
          const enrichedDocument = enrichedContract.documents[enrichedDocumentName];
          delete enrichedDocument.properties.$meta;
        }
      }
