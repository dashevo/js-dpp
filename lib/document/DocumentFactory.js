...
class DocumentFactory {
   ...
   * @param {createSubTransitions} createSubTransitions
   */
  constructor(validateDocument, fetchAndValidateDataContract, createSubTransitions) {
    this.validateDocument = validateDocument;
    this.fetchAndValidateDataContract = fetchAndValidateDataContract;
    this.createSubTransitions = createSubTransitions;
  }
   ...
   */
  createStateTransition(subTransitionsMap) {
    const documentsFlattened = Object.keys(subTransitionsMap)
      .reduce((all, t) => all.concat(subTransitionsMap[t]), []);

    if (documentsFlattened.length === 0) {
      throw new Error('No documents were supplied');
    }
    ...
    const mismatchedDocumentsLength = documentsFlattened
      .filter((document) => (
        document.getDataContractId() !== contractId || document.getOwnerId() !== ownerId
      ))
      .length;

    if (mismatchedDocumentsLength > 0) {
      throw new Error('Documents have mixed contract and owner ids');
    }

    const subTransitions = this.createSubTransitions(subTransitionsMap);
