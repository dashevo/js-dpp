...
const AbstractDocumentActionTransition = require('./stateTransition/actionTransition/AbstractDocumentActionTransition');
const DocumentCreateTransition = require('./stateTransition/actionTransition/DocumentCreateTransition');
const DocumentReplaceTransition = require('./stateTransition/actionTransition/DocumentReplaceTransition');
const DocumentDeleteTransition = require('./stateTransition/actionTransition/DocumentDeleteTransition');
...
class DocumentFactory {
  ...
  createStateTransition(documents) {
    ...
    const allowedKeys = Object.values(AbstractDocumentActionTransition.ACTIONS);
    ...
    const {
      [AbstractDocumentActionTransition.ACTIONS.CREATE]: createDocuments,
      [AbstractDocumentActionTransition.ACTIONS.REPLACE]: replaceDocuments,
      [AbstractDocumentActionTransition.ACTIONS.DELETE]: deleteDocuments,
    } = documents;

    const documentCreateTransitions = (createDocuments || [])
    ...
    const documentReplaceTransitions = (replaceDocuments || [])
    ...
    const documentDeleteTransitions = (deleteDocuments || [])
      .map((document) => new DocumentDeleteTransition({
        $id: document.getId(),
        $type: document.getType(),
      }));

    const documentActionTransitions = documentCreateTransitions
      .concat(documentReplaceTransitions)
      .concat(documentDeleteTransitions);

    return new DocumentsStateTransition({
      contractId,
      ownerId,
      transitions: documentActionTransitions.map((t) => t.toJSON()),
    });
  }
}
