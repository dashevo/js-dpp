...
const transpileEncodedProperties = require('../util/transpileEncodedProperties');
...
class Document {
  ...
  constructor(rawDocument, dataContract) {
    ...
    this.encodedProperties = this.dataContract.getEncodedProperties(
      this.getType(),
    );
  ...
  set(path, value) {
    ...
    if (Object.keys(this.encodedProperties).includes(path)) {
      const { contentEncoding } = this.encodedProperties[path];

      encodedValue = typeof value === 'string'
        ? EncodedBuffer.fromString(value, contentEncoding)
        : new EncodedBuffer(value, contentEncoding);
    }
  ...
  toJSON() {
    ...
    return transpileEncodedProperties(
      ...
      this.getType(),
      rawDocument,
      (buffer, encoding) => new EncodedBuffer(buffer, encoding).toString(),
    );
  }
  ...
  static fromJSON(rawDocument, dataContract) {
    const encodedRawDocument = transpileEncodedProperties(
      dataContract,
      rawDocument.$type,
      rawDocument,
      (string, encoding) => EncodedBuffer.fromString(string, encoding).toBuffer(),
    );

    console.log(encodedRawDocument);

    return new Document(encodedRawDocument, dataContract);
  }
}
