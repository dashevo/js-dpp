...
class Document {
    ...
    Object.keys(encodedProperties)
      .forEach((propertyPath) => {
        ...
        if (property.contentEncoding === 'binary') {
          const value = lodashGet(clonedData, propertyPath);
          if (value !== undefined) {
            lodashSet(
              clonedData,
              propertyPath,
  ...
  toObject() {
    ...
    if (this.createdAt) {
      plainObject.$createdAt = this.getCreatedAt().getTime();
    }

    if (this.updatedAt) {
      plainObject.$updatedAt = this.getUpdatedAt().getTime();
    }
 ...
 * @return {Document}
 */
Document.fromJson = (rawDocument, dataContract) => {
