...
class Document {
  ...
  constructor(rawDocument, dataContract) {
    ...
    if (Object.prototype.hasOwnProperty.call(rawDocument, '$id')) {
      this.id = Identifier.from(rawDocument.$id, Identifier.ENCODING.BASE58);
      delete data.$id;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(rawDocument, '$dataContractId')) {
      this.dataContractId = Identifier.from(
    ...
    if (Object.prototype.hasOwnProperty.call(rawDocument, '$ownerId')) {
      this.ownerId = Identifier.from(rawDocument.$ownerId, Identifier.ENCODING.BASE58);
      delete data.$ownerId;
    }
   ...
   */
  setEntropy(entropy) {
    this.entropy = Identifier.from(entropy, Identifier.ENCODING.BASE58);
  }
  ...
  set(path, value) {
    ...
    Object.entries(encodedProperties).forEach(([propertyPath]) => {
      if (path === propertyPath) {
        encodedValue = Identifier.from(value, Identifier.ENCODING.BASE64);
        ...
        if (buffer !== undefined) {
          lodashSet(
            encodedValue,
            partialPath,
            Identifier.from(buffer, Identifier.ENCODING.BASE64),
          );
        }
      }
    });
  ...
  static fromJSON(jsonDocument, dataContract) {
    const rawDocument = transpileEncodedProperties(
      ...
      jsonDocument.$type,
      jsonDocument,
      (string, encoding) => Identifier.from(string, encoding).toBuffer(),
    );
