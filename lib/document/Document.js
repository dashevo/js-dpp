...
class Document {
   ...
   */
  set(path, value) {
    const encodedValue = lodashCloneDeep(value);
    ...
    Object.entries(encodedProperties).forEach(([propertyPath, { contentEncoding }]) => {
      if (path === propertyPath) {
        lodashSet(this.data, path, EncodedBuffer.from(value, contentEncoding));
      } else if (propertyPath.includes(path)) {
        const partialPath = propertyPath.slice(0, propertyPath.indexOf(path) + path.length);
