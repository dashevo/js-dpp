...
class Document {
   ...
   */
  toJSON() {
    let rawDocument = this.toObject({ encodedBuffer: true });

    if (this.id) {
      rawDocument = {
        ...rawDocument,
        $id: rawDocument.$id.toString(),
      };
    }

    if (this.dataContractId) {
      rawDocument = {
        ...rawDocument,
        $dataContractId: rawDocument.$dataContractId.toString(),
      };
    }
    ...
    if (this.ownerId) {
      rawDocument = {
        ...rawDocument,
        $ownerId: rawDocument.$ownerId.toString(),
      };
    }
  ...
  toObject(options = {}) {
    ...
    if (!options.encodedBuffer) {
      ...
      if (this.dataContractId) {
        rawDocument = {
          ...rawDocument,
          $dataContractId: rawDocument.$dataContractId.toBuffer(),
        };
      }
      ...
      if (this.ownerId) {
        rawDocument = {
          ...rawDocument,
          $ownerId: rawDocument.$ownerId.toBuffer(),
        };
      }
