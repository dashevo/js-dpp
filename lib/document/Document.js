...
class Document {
  ...
  constructor(rawDocument) {
    ...
    if (Object.prototype.hasOwnProperty.call(rawDocument, '$meta')) {
      this.setMetadata(rawDocument.$meta);
      delete data.$meta;
    }
  ...
  setMetadata(metaObject) {
    ...
    Object.entries(metaObject)
      .forEach(([name, value]) => this.setMeta(name, value));

    return this;
  }

  /**
   * Retrieves the meta field specified by {path}
   *
   * @param {string} path
   * @return {*}
   */
  getMeta(path) {
    return lodashGet(this.meta, path);
  }

  /**
   * Set the meta field specified by {path}
   *
   * @param {string} path
   * @param {*} value
   * @return {Document}
   */
  setMeta(path, value) {
    lodashSet(this.meta, path, value);
