...
class Document {
   ...
   */
  setEntropy(entropy) {
    this.entropy = Identifier.from(entropy);
  }
  ...
  set(path, value) {
    ...
    Object.entries(encodedProperties)
      .filter(([, property]) => property.contentMediaType === 'application/x.dash.dpp.identifier')
      .forEach(([propertyPath]) => {
        if (path === propertyPath) {
          encodedValue = Identifier.from(value);
   ...
   * @param {boolean} [options.keepIdentifiers=false]
   * @return {RawDocument}
   */
  toObject(options = {}) {
    Object.assign(
