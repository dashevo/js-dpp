...
class AbstractStateTransition {
  ...
  sign(identityPublicKey, privateKey) {
    ...
    const privateKeyInHex = Buffer
      .from(privateKey, 'base64')
      .toString('hex');

    switch (identityPublicKey.getType()) {
      case IdentityPublicKey.TYPES.ECDSA_SECP256K1:
        this.signature = signData(data, privateKeyInHex).toString('base64');
        break;
      default:
        throw new InvalidSignatureTypeError(identityPublicKey.getType());
    }

    const extractedPublicKey = new PrivateKey(privateKeyInHex)
      .toPublicKey()
      .toBuffer()
      .toString('base64');

    if (extractedPublicKey !== identityPublicKey.getData()) {
