const { HashSigner: { signData, verifyDataSignature }, encoding: { Base58 } } = require('@dashevo/dashcore-lib');
...
class AbstractStateTransition {
   ...
   *
   * @param {{id: number, type: number, privateKey: string}} options
   * @return {Buffer}
   */
  sign(options) {
    const { id: publicKeyId, type, privateKey } = options;

    const data = this.serialize({ skipSignature: true });

    switch (type) {
      case signatureTypes.ECDSA:
        this.signature = Base58.encode(signData(data, privateKey));
        break;
      default:
  ...
  verifySignature(publicKeyId) {
    ...
    try {
      return verifyDataSignature(data, Base58.decode(signature), publicKeyId);
    } catch (e) {
      return false;
    }
  }
