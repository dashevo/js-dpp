...
const doubleSha = require('../../util/hash');
...
async function createDomainDataTrigger(document, context) {
  ...
  if (nameHash !== multihash.hash(doubleSha(Buffer.from(fullDomainName))).toString('hex')) {
    result.addError(
      new DataTriggerConditionError(
        document,
        context.getDataContract(),
  ...
  const parentDomainHash = multihash.hash(doubleSha(Buffer.from(normalizedParentDomainName)))
    .toString('hex');
  ...
  const saltedDomainHash = multihash.hash(doubleSha(Buffer.from(preorderSalt + nameHash, 'hex')))
    .toString('hex');
