...
async function createDomainDataTrigger(document, context) {
  ...
  if (fullDomainName.length > 253) {
    throw new Error('Full domain name length can not be more than 253 characters long');
  }
  ...
  if (fetchedTransaction === null) {
    throw new Error('dashIdentity with corresponding id was not found');
  }
  ...
  if (!isHashValidMultihash) {
    throw new Error('hash is not a valid multihash');
  }
}
