 ...
 */
async function createDomainDataTrigger(document, context, topLevelIdentity) {
  const {
    nameHash,
    label,
    normalizedLabel,
...
console.log(normalizedParentDomainName);
  if (normalizedParentDomainName.length === 0 && context.getUserId() !== topLevelIdentity) {
    result.addError(
      new DataTriggerConditionError(
        document,
        context.getDataContract(),
