...
async function createDomainDataTrigger(documentTransition, context, topLevelIdentity) {
  ...
  if (records.dashUniqueIdentityId && Buffer.compare(context.getOwnerId(), records.dashUniqueIdentityId) !== 0) {
    result.addError(
      new DataTriggerConditionError(
        documentTransition,
        context.getDataContract(),
  ...
  if (records.dashAliasIdentityId && Buffer.compare(context.getOwnerId(), records.dashAliasIdentityId) !== 0) {
    result.addError(
      new DataTriggerConditionError(
        documentTransition,
        context.getDataContract(),
  ...
  if (normalizedParentDomainName.length === 0 && Buffer.compare(context.getOwnerId().toBuffer(), topLevelIdentity) !== 0) {
    result.addError(
      new DataTriggerConditionError(
        documentTransition,
        context.getDataContract(),
  ...
  if (normalizedParentDomainName.length > 0) {
    ...
    if (parentDomain.getData().subdomainRules.allowSubdomains === false
      && Buffer.compare(context.getOwnerId(), parentDomain.getOwnerId()) !== 0) {
      result.addError(
        new DataTriggerConditionError(
          documentTransition,
          context.getDataContract(),
