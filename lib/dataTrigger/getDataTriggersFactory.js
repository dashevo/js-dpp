...
function getDataTriggersFactory() {
  ...
  if (process.env.DPNS_CONTRACT_ID && process.env.DPNS_CONTRACT_ID.length > 0) {
    dpnsDataContractId = Identifier.from(process.env.DPNS_CONTRACT_ID);
  }

  let dpnsTopLevelIdentityId;
  if (process.env.DPNS_TOP_LEVEL_IDENTITY && process.env.DPNS_TOP_LEVEL_IDENTITY.length > 0) {
