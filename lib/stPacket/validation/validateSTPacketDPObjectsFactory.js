...
const DuplicateDPObjectsByIndicesError = require('../../errors/DuplicateDPObjectsByIndicesError');
...
function validateSTPacketDPObjectsFactory(
  ...
  function validateSTPacketDPObjects(rawSTPacket, dpContract) {
    ...
    const duplicateDPObjectsByIndices = findDuplicateDPObjectsByIndices(rawDPObjects);
    if (duplicateDPObjectsByIndices.length > 0) {
      result.addError(
        new DuplicateDPObjectsByIndicesError(duplicateDPObjectsByIndices),
      );
    }
